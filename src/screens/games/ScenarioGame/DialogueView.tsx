...
const BUBBLE_STYLES = [
  {
    bubble: "border-blue-500/20 bg-blue-500/[0.08]",
    avatar: "bg-blue-500/20 text-blue-400",
  },
  {
    bubble: "border-emerald-500/20 bg-emerald-500/[0.07]",
...
export function DialogueView({ dialogue, onNext, onChangeScenario, isLoading }: Props) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
        ...
        {dialogue.turns.map((turn, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Bubble */}
              <div className={cn("max-w-[75%]", isRight && "items-end")}>
                <p
                  className={cn(
                    "text-[10px] mb-1 text-[var(--text-3)]",
                    isRight && "text-right"
                  )}
                >
                  {turn.speaker}
                ...
                >
                  ...
                  {turn.romanization && (
                    <p className="text-xs text-[var(--text-2)] mb-1 italic">
                      {turn.romanization}
                    </p>
                  )}
      ...
      {/* Footer actions */}
      <div className="px-6 py-4 border-t border-white/[0.05] flex gap-3 shrink-0">
        <button
          onClick={onChangeScenario}
          className="flex-1 py-2.5 rounded-xl border border-white/[0.07] text-sm font-medium text-[var(--text-2)] hover:bg-white/[0.04] transition-colors"
        >
          Change Scenario
        </button>
        <button
          onClick={onNext}
          disabled={isLoading}
          className="flex-1 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-white text-sm font-semibold transition-colors"
        >
          {isLoading ? "⟳ Loading…" : "New Dialogue →"}
        </button>
      </div>
    </div>
  );
